                            ...
                            <tbody>
                             ...
                             @foreach (@$join_students_regs->where('term_id', $term->id)->where('group_id', \App\StafferRegistration::where('school_year_id', '=', $schoolyear->id)->where('term_id', '=', $term->id)->where('staffer_id', \App\Staffer::where('registration_code', '=', Auth::guard('web_admin')->user()->registration_code)->first()->id)->first()->group_id ) as $key => $reg_students)
                                      ...
                                      <tr>
                                        ...
                                        @endforeach
                                        <!-- Add grade form-->
                                        <form method="post" action="{{ url('/grades/gradeactivity/student/addgrade', [$gradeactivity->id]) }}" id="addGradeForm-{{$reg_students->student->id}}" style="display: none;">
                              		      {{ csrf_field() }}
                              		   ...
                              		   @foreach ($student_grades as $student_grade)
                                            @if($student_grade->student_id == $reg_students->student->id & $student_grade->grade_activity_id == $gradeactivity->id)
		                                        <form method="post" action="{{ url('/grades/gradeactivity/student/editgrade',[$gradeactivity->id, $student_grade->id]) }}" id="editGradeForm-{{$student_grade->id}}" style="display: none;">
		                              		      {{ csrf_field() }}
		                              		      ...
		                              		      <div class="row">
		                              		        ...
		                              		        <div class="col-md-4">
		                              		        	<label>Comment</label>
		                              		      		<input type="text" class="form-control" value="{{$student_grade->activity_comment}}" name="activity_comment">
		                              		      	</div>
